...
public class Conversation {
  ...
  public static Map<Integer, Dialogue> loadDialogue(String name) {
    synchronized (cachedDialogues) {
      ...
      if (dialogue == null) {
        logger.info("Loading dialogue for " + name);
        Map<?, Dialogue> dialogue2 = Static.callScript(name + ".dialogue");
        logger.info("Loaded " + dialogue2.size() + " dialogue(s) for " + name);
  ...
  private void sendStatementDialogue(StatementDialogue dialogue) {
    ...
    int interfaceId = dialogue.pov() == POV.NONE ? 740
        /* TODO: Find correct id */ : ((dialogue.pov() == POV.NPC ? 240 : 63) + text.length);
    ...
    if (dialogue.pov() != POV.NONE) {
      Static.proto.sendString(player, interfaceId, 3,
          dialogue.pov() == POV.NPC ? npc.getDefinition().name : player.getName());
   ...
   *
   * @param option The option index relative to the text. (Can be -1 for no
   *               option)
   */
