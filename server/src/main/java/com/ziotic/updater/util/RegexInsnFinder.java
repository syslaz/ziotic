...
public class RegexInsnFinder {
    ...
    private AbstractInsnNode[] cleanInsn(AbstractInsnNode[] rawInstructions) {
        ...
        return cleanedInstructions.toArray(new AbstractInsnNode[cleanedInstructions.size()]);
    }
        ...
        for (int i = 0; i < instructions.length; i++) {
            ...
            if (n.getOpcode() >= 0) {
                if (n.getOpcode() >= opcodes.length)
                    try {
                        throw new UnexpectedException("Unknown opcode encountered: " + n.getOpcode());
                ...
                switch (n.getType()) {
                    ...
                    case AbstractInsnNode.LDC_INSN:
                        ...
                        representation += "{" + lin.cst.toString().replace("}", "\\}") + "}";
                        break;
                    ...
                    case AbstractInsnNode.FIELD_INSN:
                        ...
                        representation += "{" + fin.desc + "," + fin.owner + "," + fin.name + "}";
                        break;
                    case AbstractInsnNode.METHOD_INSN:
                        MethodInsnNode min = (MethodInsnNode) n;
                        representation += "{" + min.desc + "," + min.owner + "," + min.name + "}";
                        break;
    ...
    public AbstractInsnNode[] find(String regex) {
        try {
            Matcher regexMatcher = Pattern.compile(processRegex(regex), Pattern.MULTILINE).matcher(representation);
